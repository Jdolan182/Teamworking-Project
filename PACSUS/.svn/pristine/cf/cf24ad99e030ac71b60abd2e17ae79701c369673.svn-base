...
public class Permit_list {
	...
	public boolean getEnteredToday(String name){
		return lnkPermit.get(name).getEnteredToday();
	}
	
	
	public void resetEnteredToday(String name){
		lnkPermit.get(name).resetEnteredToday();
	}
	
	public void resetEntries(String name){
		lnkPermit.get(name).resetEnteredToday();
		lnkPermit.get(name).resetEntries();
	}


	public String getHostName(String name){
		return lnkPermit.get(name).getHostName();
	}
	

	public Permit createRegularVisitor(String name, int start, int end, String host){
		
		//might be new pw
		Regular_visitor_permit rvp = new Regular_visitor_permit(name, start, end, host);
	...
	public Permit createDayVisitor(String name, int visit, String host){
		Day_visitor_permit dvp = new Day_visitor_permit(name, visit, host);
		...
		ArrayList<String> names = new ArrayList<String>();
		 ...
		 for(int i = 0; i < allNames.size(); i++){
			if(lnkPermit.get(allNames.get(i)).getPertmitType() == "Day Visitor" || lnkPermit.get(allNames.get(i)).getPertmitType() == "Regular Visitor"){
				if((lnkPermit.get(allNames.get(i)).getEndDate().getDayNumber() <= d.getDayNumber()-1) || d.getDayNumber() == 1){
