...
public class Administration_office extends JFrame implements Observer, ActionListener {
	...
	private JLabel lblName, lblRegNo, lblMake, lblModel, lblColour, lblPermit, lblCreatedPermit, lblVehicleAssigned, lblStartDate2, lblEndDate2,
			lblDateError2, lblHost;
	private JTextField txtName, txtRegNo, txtMake, txtModel, txtColour, txtDate2, txtDate3, txtHost;
	...
	private JLabel lblName3, lblNoPermit2, lblPermitType2, lblType, lblVehiclesPermitted, lblWarningsIssued, lblWarnings3, lblIsSuspended,
			lblSuspended, lblNoOfEntries, lblEntries, lblEnteredToday, lblEntered, lblHost2, lblHostName;
	...
	public Administration_office(System_status systemStatus, Vehicle_list vehicleList, Permit_list permitList) {
	    ...
	    today = lnkSystem_status.getDate().getDayNumber();
		
		setTitle("Administration Office        Date: " + today );
		...
		pnlAddPermit.setLayout(new GridLayout(11, 2));
		...
		pnlStatusEnquiry.setLayout(new GridLayout(9, 2));
		...
		textBoxes.add(txtDate2);
		txtDate2.addKeyListener(new KeyAdapter() {
		    public void keyTyped(KeyEvent e) { 
		        if (txtDate2.getText().length() >= 3 ) 
		            e.consume(); 
		    } 
		});
		...
		textBoxes.add(txtDate3);
		txtDate3.setVisible(false);
		txtDate3.addKeyListener(new KeyAdapter() {
		    public void keyTyped(KeyEvent e) { 
		        if (txtDate3.getText().length() >= 3 ) 
		            e.consume(); 
		    } 
		});
		...
		pnlAddPermit.add(lblHost);
		txtHost = new JTextField();
		pnlAddPermit.add(txtHost);
		textBoxes.add(txtHost);
		...
		lblHost2 = new JLabel("Host name: ");
		pnlStatusEnquiry.add(lblHost2);
		lblHostName = new JLabel("");
		pnlStatusEnquiry.add(lblHostName);
		labels.add(lblHostName);
		...
		lblEnteredToday = new JLabel("Entered Today: ");
		pnlStatusEnquiry.add(lblEnteredToday);
		lblEntered = new JLabel("");
		pnlStatusEnquiry.add(lblEntered);
		labels.add(lblEntered);
	...
	private void showDate(String type){
		...
		if(type.equals("Day Visitor")){
			...
			lblStartDate2.setText("Visit Date: ");
			lblEndDate2.setText("");
			...
			txtDate2.setText(String.valueOf(today));
	...
	private void addNewPermit() {
		...
		if (((String) cmbPermitType.getSelectedItem()).equals("Day Visitor")) {
			if(validateDate(txtDate2.getText())){
				p = lnkPermit_list.createDayVisitor(txtName.getText(), Integer.parseInt(txtDate2.getText()), txtHost.getText());
			}
			else{
				lblCreatedPermit.setText("Date input error(Either date out of range or not a number)");
			}
		}

		if (((String) cmbPermitType.getSelectedItem()).equals("Regular Visitor")) {
			if(validateDate(txtDate2.getText(), txtDate3.getText())){
				p = lnkPermit_list.createRegularVisitor(txtName.getText(), Integer.parseInt(txtDate2.getText()), Integer.parseInt(txtDate3.getText()), txtHost.getText());
			}
			else{
				lblCreatedPermit.setText("Date input error(Either date out of range or not a number)");
			}
	...
	private void statusEnquiry(String name) {
		...
		if (lnkPermit_list.checkPermitted(name)) {
			...
			if (lnkPermit_list.getEnteredToday(name)) {
				entered = "Yes";
			} else {
				entered = "No";
			}
			lblEntered.setText(entered);
			
			if(type.equals("Day Visitor")){
				
				lblHostName.setText(lnkPermit_list.getHostName(name));
			} 
			else if(type.equals("Regular Visitor")){
				lblHostName.setText(lnkPermit_list.getHostName(name));
			}
	...
	private void changeDate(String name, String date, String type) {
		...
		if(type.equals("Day Visitor")){
			...
			lblDateError.setText("Date input error(Either date out of range or not a number)");
		}
	...
	private boolean validateDate(String date, String date2){
		...
		if((date.matches("^\\d+$")) && (date2.matches("^\\d+$")) ){
			if(!(Integer.parseInt(date) > lnkSystem_status.getDate().getMaxDays()) && !(Integer.parseInt(date) < lnkSystem_status.getDate().getDayNumber())
					&& !(Integer.parseInt(date2) > lnkSystem_status.getDate().getMaxDays()) && !(Integer.parseInt(date2) < lnkSystem_status.getDate().getDayNumber())){
				return true;
			}
			else{
				return false;
	...
	private boolean validateDate(String date){
		...
		if((date.matches("^\\d+$"))){
			if(!(Integer.parseInt(date) > lnkSystem_status.getDate().getMaxDays()) 
			&& !(Integer.parseInt(date) < lnkSystem_status.getDate().getDayNumber())){
				return true;
			}
			else{
				return false;
	...
	public void update(Observable arg0, Object arg1) {
		// TODO Auto-generated method stub
		Date day = lnkSystem_status.getDate();
		int d = day.getDayNumber();
		
		if(today != d){
		
		setTitle("Administration Office        Date: " +  d);
		
		
		ArrayList<String> names = lnkPermit_list.newDay(day);
		
		for(int i = 0; i < names.size(); i++){
				
			ArrayList<Vehicle_info> vi = lnkPermit_list.getVehicleInfo(names.get(i));
			lnkVehicle_list.removeAllVehiclesFromPermit(vi);
			lnkPermit_list.removePermit(names.get(i));
		}
		
		ArrayList<String> allNames = lnkPermit_list.getList();
		
		for(int i = 0; i < allNames.size(); i++){
			lnkPermit_list.resetEnteredToday(allNames.get(i));
		}
		...
		if(newYear == true && d == 1){
			...
			allNames = lnkPermit_list.getList();
			
			for(int i = 0; i < allNames.size(); i++){
				lnkPermit_list.resetEntries(allNames.get(i));
				lnkPermit_list.resetWarnings(allNames.get(i));;
			}
